$(document).ready(
    function(){
        ...
        for (let i = 0;  i < nCriterios; i++) {
            puntajesIniciales = $("#" + i + "-nbPuntaje"); //Se busca el input puntaje del criterio por defecto
            btnDeleteActual = $("#" + i + "-btnDelete");
            criterioActual = $("#" + i + "-criterio");
            //Se configura el evento de modificar puntaje en el criterio por defecto
            agregarFuncionPuntuacion(puntajesIniciales);
            if(i != 0){
                agregarFuncionDelete(btnDeleteActual, criterioActual, puntajesIniciales);
            }
        }

        colocarPuntajeTotal( puntajesIniciales );
...
//Agregar evento
function agregarFuncionPuntuacion(input){
    input.on("input",
        function(){
            colocarPuntajeTotal( input );
        }
    );
}

//Agregar Función
function agregarFuncionDelete(input, criterioActual, puntajesIniciales){
    input.on("click",
        function(){
            restarCriterio( puntajesIniciales );
            criterioActual.remove();
            disminuirCriterios(); //Se disminuye el número de criterios
        }
    );
